...
class Growtype_WC_Gateway_Paypal extends WC_Payment_Gateway
{
        ...
        if ($this->get_option('add_to_card_redirect_paypal_checkout') === 'yes' && isset($_GET['payment_method']) && $_GET['payment_method'] === self::PAYMENT_METHOD_KEY) {
            ...
            try {
                ...
                if (growtype_wc_product_is_subscription($product->get_id())) {
                    $paypal_product = $this->create_product($access_token, $product->get_id());

                    $subscription_plan = $this->create_billing_plan($access_token, $paypal_product, $order->get_id());

                    $subscription_plan_id = $subscription_plan['id'];

                    $paypal_checkout = $this->create_subscription($access_token, $subscription_plan_id, $order->get_id());

                    if (isset($paypal_checkout['id']) && !empty($paypal_checkout['id'])) {
                        $order->update_meta_data('paypal_subscription_id', $paypal_checkout['id']);
                    }
                } else {
                    $paypal_checkout = $this->create_order($access_token, $order->get_id());
                }
                ...
                if (isset($paypal_checkout['links'])) {
                    ...
                    error_log(sprintf('Something went wrong for payment provider %s. Order %s', 'paypal', $order->get_id()));
                }
            } catch (Exception $e) {
    ...
    function create_product($access_token, $product_id)
    {
        $product = wc_get_product($product_id);

        $product_url = $this->test_mode
        ...
        $body = json_encode(array (
            "name" => !empty($product_name) ? $product_name : 'Wc product',
            "description" => !empty($product_description) ? $product_description : 'Wc product description',
            "type" => "SERVICE",
            "category" => "SOFTWARE",
        ));

        $response = wp_remote_post($product_url, array (
            'headers' => $headers,
            'body' => $body,
        ));
    ...
    public function create_billing_plan($access_token, $paypal_product, $order_id)
    {
        $order = wc_get_order($order_id);
        ...
        $plan_details = array (
            ...
            "billing_cycles" => array (
                array (
                    "frequency" => array (
                        "interval_unit" => growtype_wc_get_subcription_period($wc_product->get_id()),
                        "interval_count" => growtype_wc_get_subcription_duration($wc_product->get_id())
                    ),
                    "tenure_type" => "REGULAR",
                    "sequence" => 1,
                    "total_cycles" => 0,
    ...
    function create_subscription($access_token, $plan_id, $order_id)
    {
        ...
        $subscription_data = array (
            ...
            "application_context" => array (
                ...
                "return_url" => Growtype_WC_Gateway::success_url($order_id),  // WooCommerce return URL
                "cancel_url" => Growtype_WC_Gateway::cancel_url($order_id)  // WooCommerce cart URL for cancel
            ),
//            'custom_id' => $order_id,
    ...
    public function create_order($access_token, $wc_order_id)
    {
        ...
        $order_body = [
            ...
            "application_context" => [
                "return_url" => Growtype_WC_Gateway::success_url($wc_order_id),
                "cancel_url" => Growtype_WC_Gateway::cancel_url($wc_order_id)
            ],
        ];

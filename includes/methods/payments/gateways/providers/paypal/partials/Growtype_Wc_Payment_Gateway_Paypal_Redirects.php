...
class Growtype_Wc_Payment_Gateway_Paypal_Redirects
{
    ...
    public function woocommerce_add_to_cart_extend(
        $cart_item_key,
        $wc_product_id,
        $quantity,
        $variation_id,
        ...
        $setting_enabled =
            $this->gateway->get_option(
                "add_to_card_redirect_paypal_checkout",
            ) === "yes" ||
            get_option(Growtype_Wc_Payment_Settings::OPTION_REDIRECT_PAYPAL) ===
                "yes";
        $method_match =
        ...
        if ($setting_enabled && $method_match) {
            ...
            if (!$wc_product) {
                error_log(
                    "[GWC PayPal] woocommerce_add_to_cart_extend: Product not found.",
                );
                return;
            }
            ...
            if (
                is_user_logged_in() &&
                growtype_wc_product_is_subscription($wc_product_id)
            ) {
                ...
                foreach ($existing_orders as $existing) {
                    foreach ($existing->get_items() as $item) {
                        if (
                            ...
                            error_log(
                                sprintf(
                                    "[GWC PayPal] woocommerce_add_to_cart_extend: user %d already has completed order #%d for product %d — skipping duplicate.",
                                    get_current_user_id(),
                                    $existing->get_id(),
                                    $wc_product_id,
                                ),
            ...
            if (!$order) {
                error_log(
                    "[GWC PayPal] woocommerce_add_to_cart_extend: Order creation failed.",
                );
                return;
            }
            ...
            try {
                ...
                if (
                    ...
                    $subscription_plan_id = $this->gateway->subscriptions->get_or_create_plan(
                        $access_token,
                        $wc_product->get_id(),
                        $applied_coupons,
                    );
                    ...
                    if (empty($subscription_plan_id)) {
                        error_log(
                            "[GWC PayPal] Subscription plan creation failed.",
                        );
                        throw new \Exception(
                            __(
                    ...
                    $paypal_checkout = $this->gateway->subscriptions->create_subscription(
                        $access_token,
                    ...
                    if (
                        isset($paypal_checkout["id"]) &&
                        !empty($paypal_checkout["id"])
                    ) {
                    ...
                    error_log(
                        "[GWC PayPal] woocommerce_add_to_cart_extend: Processing standard order...",
                    );
                    $paypal_checkout = $this->gateway->orders->create_order(
                        $access_token,
                        $order_id,
                        $applied_coupons,
                        "paypal",
                ...
                error_log(
                    sprintf(
                        "[GWC PayPal] add_to_cart error: %s",
                        $e->getMessage(),
                    ),
                );
                $order->update_status(
    ...
    public function payment_redirect()
    {
        ...
        if (is_user_logged_in()) {
            ...
            if (
                $order_customer_id > 0 &&
                $order_customer_id !== get_current_user_id()
            ) {
                error_log(
                    sprintf(
                        "[GWC PayPal] payment_redirect: ownership check failed for order %d — customer %d vs current user %d",
                        $order_id,
                        $order_customer_id,
                        get_current_user_id(),
                    ),
        ...
        if (
            $payment_method === Growtype_Wc_Payment_Gateway_Paypal::PROVIDER_ID
        ) {
            ...
            if (Growtype_Wc_Subscription::is_subscription_order($order_id)) {
                ...
                $stored_hosted_token = $order->get_meta(
                    "_paypal_hosted_order_id",
                );

                if (
                    $paypal_order_id !== $stored_token &&
                    $paypal_order_id !== $stored_hosted_token
                ) {
                    error_log(
                        sprintf(
            ...
            $is_subscription = Growtype_Wc_Subscription::is_subscription_order(
                $order_id,
            );
            ...
            if ($is_subscription) {
                ...
                $paypal_order_data = $this->gateway->subscriptions->get_subscription_data(
                    $access_token,
                    $subscription_id,
                );
            } else {
            ...
            $customer_email =
                $paypal_order_data["payer"]["email_address"] ?? "";
            if (
                empty($customer_email) &&
                isset($paypal_order_data["subscriber"]["email_address"])
            ) {
                $customer_email =
                    $paypal_order_data["subscriber"]["email_address"];
            }
            if (empty($customer_email)) {
                $customer_email = Growtype_Wc_Payment_Gateway::resolve_user_email();
            }
            ...
            if (isset($paypal_order_data["status"])) {
                $is_approved = false;
                if ($is_subscription) {
                    $is_approved = in_array($paypal_order_data["status"], [
                        "ACTIVE",
                        "APPROVED",
                    ]);
                } else {
                ...
                if ($is_approved) {
                    ...
                    if ($paypal_order_data["status"] === "APPROVED") {
                        $order->add_order_note(
                            __(
                                sprintf("Order id: %s", $paypal_order_id),
                                "growtype-wc",
                            ),
                        );

                        if (
                            isset($paypal_order_data["intent"]) &&
                            $paypal_order_data["intent"] === "CAPTURE"
                        ) {
                            foreach ($paypal_order_data["links"] as $link) {
                                if ($link["rel"] === "capture") {
                                    $capture_data = $this->gateway->orders->capture_order(
                                        $access_token,
                                        $paypal_order_id,
                                    );
                                    break;
                    ...
                    error_log(
                        sprintf(
                            "[GWC Vault] payment_redirect full capture_data payment_source: %s",
                            wp_json_encode(
                                $capture_data["payment_source"] ?? null,
                            ),
                        ),
                    );
                    error_log(
                        sprintf(
                            "[GWC Vault] payment_redirect full capture_data status: %s",
                            $capture_data["status"] ?? "n/a",
                        ),
                    );
                    if (!empty($capture_data["purchase_units"])) {
                        foreach (
                            $capture_data["purchase_units"]
                            as $pu_idx => $pu
                        ) {
                    ...
                    $vault_id =
                        $capture_data["payment_source"]["paypal"]["attributes"][
                            "vault"
                        ]["id"] ?? "";
                    $pp_customer =
                        $capture_data["payment_source"]["paypal"]["attributes"][
                    ...
                    if (empty($vault_id)) {
                        $vault_id =
                            $capture_data["payment_source"]["card"][
                                "attributes"
                            ]["vault"]["id"] ?? "";
                        $pp_customer =
                            $capture_data["payment_source"]["card"][
                    ...
                    if (
                        empty($vault_id) &&
                        !empty($capture_data["purchase_units"])
                    ) {
                        foreach ($capture_data["purchase_units"] as $pu) {
                            foreach (
                                ...
                                $v =
                                    $cap["payment_source"]["paypal"][
                                        "attributes"
                                    ]["vault"]["id"] ??
                                    ($cap["payment_source"]["card"][
                                        "attributes"
                                    ]["vault"]["id"] ??
                                        "");
                                $c =
                                ...
                                if (!empty($v)) {
                                    ...
                                    error_log(
                                        sprintf(
                                            "[GWC Vault] Found vault_id in purchase_units captures: %s customer: %s",
                                            $vault_id,
                                            $pp_customer,
                                        ),
                                    );
                    ...
                    if (empty($vault_id)) {
                        ...
                        if (!empty($vault_id)) {
                            error_log(
                                sprintf(
                                    "[GWC Vault] Using vault_id from order meta (set during create_order): %s",
                                    $vault_id,
                                ),
                            );
                        }
                    ...
                    error_log(
                        sprintf(
                            "[GWC Vault] payment_redirect final result: order=%d vault_id=%s pp_customer=%s",
                            $order_id,
                            $vault_id,
                            $pp_customer,
                        ),
                    ...
                    if (!empty($pp_customer)) {
                        $order->update_meta_data(
                            "paypal_customer_id",
                            sanitize_text_field($pp_customer),
                        );
                        if ($order->get_customer_id() > 0) {
                            ...
                            error_log(
                                sprintf(
                                    "[GWC Vault] Saved paypal_customer_id=%s to WP user %d",
                                    $pp_customer,
                    ...
                    if (empty($vault_id)) {
                        error_log(
                            '[GWC Vault] ⚠ vault_id still empty after all path checks — PayPal did not return vault data. Ensure "Vault" is enabled in PayPal PPCP settings and the merchant account supports Reference Transactions.',
                        );
                    }

                    if (
                        Growtype_Wc_Subscription::is_subscription_order(
                            $order_id,
                        )
                    ) {
                    ...
                    if (defined("WP_DEBUG") && WP_DEBUG) {
                        error_log(
                            sprintf(
                                "Order %s is not approved. PayPal order id: %s. Status: %s.",
                                $order_id,
                                $paypal_order_id,
                                $paypal_order_data["status"] ?? "unknown",
                            ),
                        );
                    }
                }
                ...
                if (defined("WP_DEBUG") && WP_DEBUG) {
                    error_log(
                        sprintf(
                            "Order %s is not paid and status is missing. PayPal order id: %s.",
                            $order_id,
                            $paypal_order_id,
                        ),
                    );

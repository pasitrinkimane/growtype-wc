...
function woocommerce_account_extend_purchased_products_endpoint()
{
     ...
     */
    $products_ids = Growtype_Wc_Product::get_user_purchased_products_ids(get_current_user_id(), array_keys($product_types));

    $products_ids = !empty($products_ids) ? implode(',', $products_ids) : null;

    echo growtype_wc_include_view('woocommerce.myaccount.purchased-products', ['products_ids' => $products_ids]);
}

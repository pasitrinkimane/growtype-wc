...
class Custom_WC_Widget_Product_Categories extends WC_Widget
{
    ...
    public function __construct()
    {
        ...
        $this->widget_description = __('A list or dropdown of product categories.', 'growtype-wc');
        $this->widget_id = 'woocommerce_product_categories';
        $this->widget_name = __('Growtype - Filter Products by Category', 'growtype-wc');
        $this->settings = array (
            'title' => array (
                'type' => 'text',
                'std' => __('Product categories', 'growtype-wc'),
                'label' => __('Title', 'growtype-wc'),
            ),
            'orderby' => array (
                'type' => 'select',
                'std' => 'name',
                'label' => __('Order by', 'growtype-wc'),
                'options' => array (
                    'menu_order' => __('Menu order', 'growtype-wc'),
                    'name' => __('Name', 'growtype-wc'),
                ),
            ),
            'dropdown' => array (
                'type' => 'checkbox',
                'std' => 0,
                'label' => __('Show as dropdown', 'growtype-wc'),
            ),
            'count' => array (
                'type' => 'checkbox',
                'std' => 0,
                'label' => __('Show product counts', 'growtype-wc'),
            ),
            'hierarchical' => array (
                'type' => 'checkbox',
                'std' => 1,
                'label' => __('Show hierarchy', 'growtype-wc'),
            ),
            'show_children_only' => array (
                'type' => 'checkbox',
                'std' => 0,
                'label' => __('Only show children of the current category', 'growtype-wc'),
            ),
            'hide_empty' => array (
                'type' => 'checkbox',
                'std' => 0,
                'label' => __('Hide empty categories', 'growtype-wc'),
            ),
            'hide_uncategorized' => array (
                'type' => 'checkbox',
                'std' => 0,
                'label' => __('Hide uncategorized', 'growtype-wc'),
            ),
            'allow_multiple' => array (
                'type' => 'checkbox',
                'std' => 0,
                'label' => __('Allow multiple categories', 'growtype-wc'),
            ),
            'multiple_include_parent' => array (
                'type' => 'checkbox',
                'std' => 0,
                'label' => __('Multiple categories include parent category', 'growtype-wc'),
            ),
            'allow_children_collapse' => array (
                'type' => 'checkbox',
                'std' => 0,
                'label' => __('Allow children elements collapse', 'growtype-wc'),
            ),
            'max_depth' => array (
                'type' => 'text',
                'std' => '',
                'label' => __('Maximum depth', 'growtype-wc'),
            ),
        );
    ...
    public function widget($args, $instance)
    {
        ...
        if ($dropdown) {
				...
				if ( jQuery().selectWoo ) {
					var wc_product_cat_select = function() {
						jQuery( '.dropdown_product_cat' ).selectWoo( {
							placeholder: '" . esc_js(__('Select a category', 'growtype-wc')) . "',
							minimumResultsForSearch: 5,
							width: '100%',
							allowClear: true,
							language: {
								noResults: function() {
									return '" . esc_js(_x('No matches found', 'enhanced select', 'growtype-wc')) . "';
								}
							}
						} );
					};
            ...
            $list_args['show_option_none'] = __('No product categories exist.', 'growtype-wc');
            ...
            if ($allow_multiple) {
                echo '<button class="btn btn-sm btn-clear-cat-filter mt-3">' . __('Clear selected options', 'growtype-wc') . '</button>';
            }
        }

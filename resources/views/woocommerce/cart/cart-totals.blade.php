...
<div class="cart_totals <?php echo ( WC()->customer->has_calculated_shipping() ) ? 'calculated_shipping' : ''; ?>">
	...
	<p class="e-title-section"><?php esc_html_e( 'Total', 'growtype-wc' ); ?></p>

	<div class="shop_table shop_table_responsive">

		<div class="cart-subtotal">
			<span><?php esc_html_e( 'Subtotal', 'growtype-wc' ); ?></span>
			<span data-title="<?php esc_attr_e( 'Subtotal', 'growtype-wc' ); ?>"><?php wc_cart_totals_subtotal_html(); ?></span>
		</div>
		...
		<?php elseif ( WC()->cart->needs_shipping() && 'yes' === get_option( 'woocommerce_enable_shipping_calc' ) ) : ?>
			...
			<div class="shipping">
				<span><?php esc_html_e( 'Shipping', 'growtype-wc' ); ?></span>
				<span data-title="<?php esc_attr_e( 'Shipping', 'growtype-wc' ); ?>"><?php woocommerce_shipping_calculator(); ?></span>
			</div>
		...
		if ( wc_tax_enabled() && ! WC()->cart->display_prices_including_tax() ) {
			...
			if ( WC()->customer->is_customer_outside_base() && ! WC()->customer->has_calculated_shipping() ) {
				...
				$estimated_text = sprintf( ' <small>' . esc_html__( '(estimated for %s)', 'growtype-wc' ) . '</small>', WC()->countries->estimated_for_prefix( $taxable_address[0] ) . WC()->countries->countries[ $taxable_address[0] ] );
			}
		...
		<div class="order-total">
			<span><?php esc_html_e( 'Total', 'growtype-wc' ); ?></span>
			<span data-title="<?php esc_attr_e( 'Total', 'growtype-wc' ); ?>"><?php wc_cart_totals_order_total_html(); ?></span>
		</div>
